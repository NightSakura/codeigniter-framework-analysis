CI框架提供了钩子的方法来修改框架的内部运作流程，而无需修改核心文件。CodeIgniter 的运行遵循着一个特定的流程，但通过Hook特性你可能希望在 执行流程中的某些阶段添加一些动作，可以简单理解为主流程执行过程中预留了交给开发者实现的API。钩子的实现在文件`Hooks.php`文件中，类名为CI_Hooks。
